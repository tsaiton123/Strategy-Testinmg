from __future__ import annotations
import numpy as np
import gymnasium as gym
from gymnasium import spaces
import pandas as pd
from .utils import build_features, to_numpy_windowed, align_prices_for_obs, normalize_features, select_stable_features


class SingleAssetTradingEnv(gym.Env):
    """
    改良版 RL Trading Environment
    Reward = 盈利能力 + 違規懲罰 + drawdown penalty
    Constraint = FTMO 規則
    """

    metadata = {"render_modes": []}

    def __init__(self,
                 df: pd.DataFrame,
                 window: int = 64,
                 action_mode: str = "box",
                 max_leverage: float = 30.0,
                 cost_bps: float = 15.0,
                 reward_scale: float = 1.0,  # ✅改大，原本 0.1 太小
                 action_tau: float = 0.1,
                 hold_min: int = 5,
                 initial_balance: float = 100000.0,
                 daily_loss_limit: float = -5000.0,
                 max_total_loss: float = -10000.0):

        super().__init__()
        # ===== Data & Features 初始化不變 =====
        x = df.sort_values("ts").copy()
        x["ts"] = pd.to_datetime(x["ts"], utc=True, errors="coerce")
        x = x.dropna(subset=["ts"])
        feats = build_features(x)
        feats = select_stable_features(feats)
        feats = normalize_features(feats, method='robust')

        self.px = x.set_index("ts")["close"].astype(float).reindex(feats.index).ffill()
        self.obs_3d = to_numpy_windowed(feats, window)
        self.rets = align_prices_for_obs(self.px, window)
        self.window = window
        self.cost = float(cost_bps) / 1e4

        self.reward_scale = float(reward_scale)
        self.action_tau = float(action_tau)
        self.hold_min = int(hold_min)

        # ===== Account state =====
        self.initial_balance = float(initial_balance)
        self.daily_loss_limit = float(daily_loss_limit)
        self.max_total_loss = float(max_total_loss)
        self.balance = self.initial_balance
        self.daily_start_balance = self.initial_balance
        self._hold = 0

        self.returns_history = []
        self.equity_history = [self.balance]
        self.peak_equity = self.balance

        # ===== Action/Observation space 不變 =====
        T, W, F = self.obs_3d.shape if self.obs_3d.size else (0, window, feats.shape[1])
        self._T, self._F = T, F

        if action_mode == "discrete":
            self.action_space = spaces.Discrete(3)
            self._discrete = True
        else:
            self.action_space = spaces.Box(low=-max_leverage, high=max_leverage, shape=(1,), dtype=np.float32)
            self._discrete = False

        self.observation_space = spaces.Box(low=-np.inf, high=np.inf, shape=(W*F,), dtype=np.float32)

        # Internal state for reward normalization
        self._reward_running_mean = 0.0
        self._reward_running_var = 1.0
        self._reward_count = 1e-8

    # ===== Reset =====
    def reset(self, *, seed: int | None = None, options=None):
        super().reset(seed=seed)
        self._t = 0
        self._pos = 0.0
        self.returns_history = []
        self.equity_history = [self.initial_balance]
        self.balance = self.initial_balance
        self.daily_start_balance = self.initial_balance
        self.peak_equity = self.initial_balance
        self._hold = 0
        if self._T == 0:
            raise RuntimeError("Not enough data")
        obs = self.obs_3d[self._t].reshape(-1).astype(np.float32)
        return obs, {}

    # ===== Step =====
    def step(self, action):
        # --- Map action ---
        if self._discrete:
            a = int(action)
            target = [-1,0,1][a] * self.action_space.high
        else:
            target = float(np.clip(action, -self.action_space.high, self.action_space.high))

        # Smooth position
        target = self.action_tau * target + (1.0 - self.action_tau) * self._pos
        if self._hold > 0:
            target = self._pos
            self._hold -= 1
        else:
            if abs(target - self._pos) > 1e-12:
                self._hold = max(self.hold_min - 1, 0)

        turnover = abs(target - self._pos)
        cost = self.cost * turnover

        # Portfolio return
        portfolio_return = self._pos * self.rets[self._t] - cost
        self.balance *= (1 + portfolio_return)

        self.returns_history.append(portfolio_return)
        self.equity_history.append(self.balance)
        self.peak_equity = max(self.peak_equity, self.balance)

        # --- Constraint check ---
        daily_loss = self.balance - self.daily_start_balance
        total_loss = self.balance - self.initial_balance
        violated = daily_loss < self.daily_loss_limit or total_loss < self.max_total_loss

        # --- Reward ---
        reward = self._calculate_risk_adjusted_reward(portfolio_return, turnover, daily_loss, total_loss, violated)  # ✅改動：將違規納入 reward

        self._pos = target
        self._t += 1

        # ✅改動：違規也會終止 episode
        terminated = self._t >= self._T or violated
        truncated = False

        obs = np.zeros_like(self.obs_3d[0].reshape(-1), dtype=np.float32) if terminated else self.obs_3d[self._t].reshape(-1).astype(np.float32)

        info = {"position": self._pos, "equity": self.balance,
                "daily_loss": daily_loss, "total_loss": total_loss, "violated": violated}

        return obs, float(reward), terminated, truncated, info

    # ===== Reward function =====
    def _calculate_risk_adjusted_reward(self, portfolio_return, turnover, daily_loss, total_loss, violated):
        # Base reward
        base_reward = portfolio_return

        # Turnover penalty
        turnover_penalty = -turnover * self.cost * 2

        # Drawdown penalty
        current_equity = self.equity_history[-1]
        drawdown = (current_equity - self.peak_equity) / self.peak_equity
        drawdown_penalty = min(0, drawdown * 10)  # ✅加大懲罰力度

        # Risk-adjusted return with shorter rolling window
        window_len = 20  # ✅原本 50 -> 20
        if len(self.returns_history) >= window_len:
            rolling_returns = np.array(self.returns_history[-window_len:])
            rolling_mean = np.mean(rolling_returns)
            rolling_std = np.std(rolling_returns) + 1e-8
            risk_adjusted_return = (portfolio_return - rolling_mean) / rolling_std
        else:
            risk_adjusted_return = portfolio_return

        raw_reward = risk_adjusted_return + turnover_penalty + drawdown_penalty

        # --- 違規懲罰加入 reward ---
        if violated:
            raw_reward -= 5.0 + abs(daily_loss) / max(1.0, abs(self.daily_loss_limit))

        # Online normalization
        self._reward_count += 1
        delta = raw_reward - self._reward_running_mean
        self._reward_running_mean += delta / self._reward_count
        self._reward_running_var += delta * (raw_reward - self._reward_running_mean)

        std = np.sqrt(self._reward_running_var / self._reward_count)
        if std < 1e-8: std = 1.0

        normalized_reward = (raw_reward - self._reward_running_mean) / std

        return normalized_reward * self.reward_scale  
